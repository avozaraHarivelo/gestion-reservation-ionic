...
export class CalendarComponent implements OnInit {
  ...
  constructor(private dialog: MatDialog, private roomService: RoomService, private reservationService: ReservationService) {
    ...
    this.reservationService.getBookings().subscribe((bookings: Booking[]) => {
      ...
      this.updateCalendar(this.cellHeight, this.cellWidthDay); // Appelez la mise à jour du calendrier une fois que vous avez les réservations
    });
  }
  ...
  ngOnInit() {
    ...
    this.updateCalendar(this.cellHeight, this.cellWidthDay);
  ...
  onPrevMonth() {
    ...
    this.updateCalendar(this.cellHeight, this.cellWidthDay); // Mettre à jour le calendrier avec le mois et l'année actuels
  }
  ...
  updateCalendar(cellHeight: number, cellWidth: number) {
    // console.log(this.bookings)

    // Variables locales pour stocker les valeurs globales
    const cellWidthRoom = this.cellWidthRoom;
    const cellWidthDay = cellWidth;



    var yearDays = this.getDaysInYear(this.currentYear);

    var monthName = [
      'Janvier', 'Février', 'Mars', 'Avril', 'Mai', 'Juin', 'Juillet', 'Août', 'Septembre', 'Octobre', 'Novembre', 'Décembre'
    ];
    // this.cellWidthDay = Math.ceil((window.innerWidth - this.cellWidthRoom * 3) / 14)
    // Calculer la largeur du canvas en fonction du nombre de jours dans l'année
    var canvasWidth = this.cellWidthRoom * 3 + (cellWidthDay * yearDays); // Largeur de la colonne des noms des chambres + largeur des jours du mois
    ...
    // Obtenir le dernier jour de la dernière semaine de l'année précédente
    const lastDayOfPrevYearWeek = new Date(this.currentYear - 1, 11, 31).getDay();

    // Calculer le nombre de jours dans la première semaine de janvier de l'année courante
    const daysInFirstWeekOfJanuary = 7 - lastDayOfPrevYearWeek;


    // ... Le reste du code ...
    // Obtenir le dernier jour de la dernière semaine de l'année précédente
    const dernierJourAnneePrecedente = new Date(this.currentYear - 1, 11, 31).getDay();
    console.log(`dernierJourAnneePrecedente :${dernierJourAnneePrecedente}`)

    // Calculer le nombre de jours dans la première semaine de janvier de l'année courante
    const joursDansPremiereSemaineJanvier = 7 - dernierJourAnneePrecedente;
    console.log(`joursDansPremiereSemaineJanvier :${joursDansPremiereSemaineJanvier}`)

    // Définir la position de départ (x) à partir de cellWidthRoom * 3
    let positionX = this.cellWidthRoom * 3;
    let positionXSemaine = this.cellWidthRoom * 3;
    ...
    monthName.forEach((mois, index) => {
      ...
      var monthDays = this.getDaysInMonth(this.currentYear, index);
      var joursDansMois = this.getDaysInMonth(this.currentYear, index);
      console.log(`joursDansMois :${joursDansMois}`)
      // Créer une cellule pour représenter le mois
      var celluleMois = new Konva.Rect({
        width: joursDansMois * cellWidthDay - 1,
        height: 30 - 1,
        fill: '#f0f0f0',
        stroke: 'black',
        strokeWidth: 1,
      ...
      var enTeteMois = new Konva.Text({
        ...
        width: joursDansMois * cellWidthDay,
        height: 30,
        align: 'center',
        verticalAlign: 'middle',
        fontSize: 18,
      ...
      positionX += joursDansMois * cellWidthDay;
      ...
      console.log(`premierJourDuMois :${premierJourDuMois}`);




      // Remap des jours pour commencer par lundi (lundi = 0, dimanche = 6)
      const premierJourDuMoisLundi = premierJourDuMois === 0 ? 6 : premierJourDuMois - 1;



      console.log(`premierJourDuMoisLundi :${premierJourDuMoisLundi}`);
      const semainesDansMois = Math.ceil((joursDansMois + premierJourDuMois) / 7);

      console.log(`semainesDansMois :${semainesDansMois}`);
      ...
      for (let semaine = 0; semaine < semainesDansMois; semaine++) {
        ...
        console.log(`numeroSemaine  ${numeroSemaine}`, premierJourSemaine)
        //Premier semaine de janvier
        if (index == 0 && semaine == 0) {
          // positionXSemaine += premierJourDuMoisLundi * cellWidthDay;
          var celluleSemaine = new Konva.Rect({
            width: (7 - premierJourDuMoisLundi) * cellWidthDay,
            height: cellHeight - 1,
            fill: '#f0f0f0',
            stroke: 'black',
            strokeWidth: 1,
          ...
          var texteSemaine = new Konva.Text({
            text: `1/ ${this.currentYear}`, // Vous pouvez personnaliser le texte ici si nécessaire
            width: (7 - premierJourDuMoisLundi) * cellWidthDay,
            height: cellHeight,
            align: 'left',
            verticalAlign: 'middle',
          ...
          positionXSemaine += (7 - premierJourDuMoisLundi) * cellWidthDay
        } else {

          // Créer une cellule pour représenter la semaine
          var celluleSemaine = new Konva.Rect({
            width: 7 * cellWidthDay - 1,
            height: cellHeight - 1,
            fill: '#f0f0f0',
            stroke: 'black',
            strokeWidth: 1,
          ...
          var texteSemaine = new Konva.Text({
            text: `${numeroSemaine}/ ${this.currentYear}`, // Vous pouvez personnaliser le texte ici si nécessaire
            width: 7 * cellWidthDay,
            height: cellHeight,
            align: 'left',
            verticalAlign: 'middle',
            fontSize: 16,
          ...
          positionXSemaine += 7 * cellWidthDay
        }
        ...
        for (let col = 1; col <= monthDays; col++) {
          ...
          cell.position({
            x: this.cellWidthRoom * 3 + (col - 1) * cellWidthDay + positionJour,
            y: 30 + cellHeight, // À partir de la ligne 30 pour l'en-tête des jours du mois
          });

          var currentDate = new Date(this.currentYear, index, col);
      ...
      positionJour += monthDays * cellWidthDay;
    });
    ...
    for (let row = 1; row <= this.numRooms; row++) {
      ...
      for (let col = 0; col <= yearDays; col++) {
        var cell = new Konva.Rect({
    ...
    for (let row = this.numRooms; row >= 1; row--) {
      ...
      var t = new Konva.Transformer({
        ...
        boundBoxFunc: (oldBox, newBox) => {
          ...
          this.updateCalendar(newBox.height, this.cellWidthDay)
    ...
    for (let cell = yearDays; cell >= 0; cell--) {
      console.log(`jours :${yearDays} position: ${this.cellWidthRoom * 3 + (cell) * this.cellWidthDay + positionJour}`)

      var day = new Konva.Rect({
        width: this.cellWidthDay - 1,
        height: cellHeight - 1,
        opacity: 0.1,
        strokeWidth: 1,
        className: 'days-cell'
      ...
      day.position({
        x: this.cellWidthRoom * 3 + (cell) * this.cellWidthDay,
        y: 30 + cellHeight, // À partir de la ligne 30 pour l'en-tête des jours du mois
      });
      // create new transformer
      var t = new Konva.Transformer({
        ...
        boundBoxFunc: (oldBox, newBox) => {
          // Obtenir la nouvelle hauteur
          this.cellWidthDay = newBox.width;

          this.updateCalendar(this.cellHeight, newBox.width)
